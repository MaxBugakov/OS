...
void notify_observers(const char *message, int sock) {
    pthread_mutex_lock(&observers_mutex);
    struct observer *obs = observers;
    while (obs != NULL) {
        sendto(sock, message, strlen(message), 0, (struct sockaddr *)&obs->addr, obs->addr_len);
        obs = obs->next;
    }
...
void handle_client(char *buffer, struct sockaddr_in *client_addr, int sock, socklen_t client_len) {
    ...
    if (strcmp(token, "reader") == 0) {
        ...
        notify_observers(response, sock);
