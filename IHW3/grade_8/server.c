...
observer_node_t* observers_head = NULL;
...
void add_observer(int observer_socket) {
    pthread_mutex_lock(&observers_mutex);
    observer_node_t* new_node = (observer_node_t*)malloc(sizeof(observer_node_t));
    new_node->socket = observer_socket;
    new_node->next = observers_head;
    observers_head = new_node;
    pthread_mutex_unlock(&observers_mutex);
}

void remove_observer(int observer_socket) {
    pthread_mutex_lock(&observers_mutex);
    observer_node_t* temp = observers_head;
    observer_node_t* prev = NULL;
    
    while (temp != NULL && temp->socket != observer_socket) {
        prev = temp;
        temp = temp->next;
    }
...
int main(int argc, char const *argv[]) {
    ...
    pthread_mutex_init(&observers_mutex, NULL);
    sem_init(&db_sem, 0, 1);
    ...
    pthread_mutex_destroy(&observers_mutex);
    sem_destroy(&db_sem);
    return 0;
}
