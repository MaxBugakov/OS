...
int main(int argc, char *argv[]) {
    ...
    if (pid1 == 0) {
        // Процесс 1: чтение из файла и отправка через FIFO1
        ...
        if (pid2 == 0) {
            // Процесс 2: обработка данных и отправка через FIFO2
            ...
            ssize_t bytes_read = read(fifo2_fd, buffer, sizeof(buffer));
            if (bytes_read == -1) {
